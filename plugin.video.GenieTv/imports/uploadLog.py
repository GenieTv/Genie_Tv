...
import urllib2
import xbmc
from imports import uservar 

from xbmc import getCondVisibility as condition, translatePath as translate, log as xbmc_log

DIALOG        = xbmcgui.Dialog()
AddonID       = uservar.ADDON_ID
addon         = uservar.addonId(AddonID)
ADDON_TITLE   = uservar.addonInfo(AddonID,'name')
ADDON_VERSION = uservar.addonInfo(AddonID,'version')
DEBUG         = False
...
class LogUploader(object):
    ...
    def get_settings(self):
        self.email_address = uservar.getS('email')
        self.__log('settings: len(email)=%d' % len(self.email_address))
        self.skip_oldlog = uservar.getS('skip_oldlog') == 'true'
