...
for these amazing sections all issues with errors in this addon GenieTv should be reported to @Origin_Ent but streams not working taken to original 
author/playlister'''
...
ADDON_DATA = USERDATA_PATH + '/GenieTv/'
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(GenieTv ,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(GenieTv ,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
				
				
def SKindex():
    addon_log("SKindex")
    addDir('[B][COLOR gold]THE PYRAMID SEARCH[/B][/COLOR]','https://archive.org/download/tombebbington4_virginmedia_Home/moviesa-z.txt',1141000,'http://previews.123rf.com/images/markinv/markinv1212/markinv121200020/17010740-All-seeing-eye-Stock-Vector-horus-eye-egyptian.jpg' ,  FANART,'','','','')
...
def SKindex_TigensWorld():
    addDir('Search Tigens World Movies','MULTILINK-TIGEN',1141000,'http://herovision.x10host.com/freeview/Tigen.png' ,  FANART,'','','','')
...
def WizHDMenu(url,iconimage,fanart):
	addDir('Paul\'s Picks',url,1101000,'',fanart,'','','',None,'source')
	...
	for item in block:
		...
		for url2,iconimage,name in match:
			name = name.title()
			image = iconimage
			addDir(name.encode('utf-8'),url2.encode('utf-8'),1136000,image,fanart,'','','',None,'source')
		...
		if date == fin_date:
			...
			for time,teams,link_block in match:
				...
				if len(List)>1:
					...
					xbmc.log(vurl)
					addDir(time.replace(' ','')+'-'+name,vurl,1135000,icon,FANART,'','','','')
					del List[:]
...
def scrape():
    ...
    for name,url in match:
		PLAY(name,url,1138000,'http://goo.gl/TyDD6w' ,FANART,'','')
...
def PLAY(name, url, mode, iconimage, fanart, description, extra, showcontext=True, allinfo={}):
    ...
    if showcontext:
        contextMenu = []
        if showcontext == 'fav':
            contextMenu.append(('Remove from GenieTv Favorites', 'XBMC.RunPlugin(%s?mode=12&name=%s)'
                                % (sys.argv[0], urllib.quote_plus(name))))
        if not name in FAV:
            contextMenu.append(('Add to GenieTv Favorites',
        ...
        for item in List:
            if 'boxset' in item:
                ...
                for name,image,url,fanart in match:
                    if (Search_name).replace(' ','').lower() in (name).replace(' ','').lower():
                        addDir(name,url,1101000,image,fanart,'','','','')
            else:
				HTML = Open_Url(item)
				if HTML != 'Failed':
...
def getSources():
        if os.path.exists(favorites) == True:
            addDir('Favorites','url',1104000,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
        if addon.getSetting("browse_xml_database") == "true":
            addDir('XML Database','http://xbmcplus.xb.funpic.de/www-data/filesystem/',1115,icon,FANART,'','','','')
        if addon.getSetting("browse_community") == "true":
            addDir('Community Files','community_files',1116000,icon,FANART,'','','','')
        if os.path.exists(history) == True:
            addDir('Search History','history',1125000,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
        if addon.getSetting("searchyt") == "true":
            addDir('Search:Youtube','youtube',1125000,icon,FANART,'','','','')
        if addon.getSetting("searchDM") == "true":
            addDir('Search:dailymotion','dmotion',1125000,icon,FANART,'','','','')
        if addon.getSetting("PulsarM") == "true":
            addDir('Pulsar:IMDB','IMDBidplay',1127000,icon,FANART,'','','','')            
        if os.path.exists(source_file)==True:
            ...
            if len(sources) > 1:
                for i in sources:
                    ...
                    if isinstance(i, list):
                        addDir(i[0].encode('utf-8'),i[1].encode('utf-8'),1101000,icon,FANART,'','','','','source')
                    else:
                        thumb = icon
                        fanart = FANART
                        desc = ''
                        ...
                        if i.has_key('credits'):
                            credits = i['credits']
                        addDir(i['title'].encode('utf-8'),i['url'].encode('utf-8'),1101000,thumb,fanart,desc,genre,date,credits,'source')
...
def addSource(url=None):
        ...
        if not url is None:
            if 'xbmcplus.xb.funpic.de' in url:
                xbmc.executebuiltin("XBMC.Container.Update(%s?mode=1114000,replace)" %sys.argv[0])
            elif 'community-links' in url:
                xbmc.executebuiltin("XBMC.Container.Update(%s?mode=1110000,replace)" %sys.argv[0])
        ...
        for i in soup('a'):
            ...
            if not href.startswith('?'):
                ...
                if name not in ['Parent Directory', 'recycle_bin/']:
                    if href.endswith('/'):
                        if browse:
                            addDir(name,url+href,1115000,icon,fanart,'','','')
                        else:
                            addDir(name,url+href,1114000,icon,fanart,'','','')
                    elif href.endswith('.xml'):
                        if browse:
                            addDir(name,url+href,1101000,icon,fanart,'','','','','download')
                        else:
                            if os.path.exists(source_file)==True:
                                if name in SOURCES:
                                    addDir(name+' (in use)',url+href,1111000,icon,fanart,'','','','','download')
                                else:
                                    addDir(name,url+href,1111000,icon,fanart,'','','','','download')
                            else:
                                addDir(name,url+href,1111000,icon,fanart,'','','','','download')
...
def getCommunitySources(browse=False):
        ...
        for i in files:
            name = i('a')[0]['href']
            if browse:
                addDir(name,url+name,1101000,icon,fanart,'','','','','download')
            else:
                addDir(name,url+name,1111000,icon,fanart,'','','','','download')
                ...
                try:
                    if linkedUrl=='':
                        if Adult_Pass == Adult_Default:
                            addDir(url,url.encode('utf-8'),1102000,thumbnail,fanArt,desc,genre,date,credits,True)
                        else:
                        ...
                        if 'Have a nice day now' in linkedUrl:
                            ...
                            Random_play('Play 10 Random Cartoons',1154000,url='http://raiztv.co.uk/RaysRavers/list2/raiztv/kids/kidsrandom.txt',image=thumbnail,isFolder=False)
                        elif name == 'Live Events':
                            addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),1135000,thumbnail,fanArt,desc,genre,date,None,'source')
                        else:						
                            if Adult_Pass == Adult_Default:
                                addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),1101000,thumbnail,fanArt,desc,genre,date,None,'source')
                            else:
                                if 'xxx' in name.lower():
                                    pass
                                elif 'adult' in name.lower():
    ...
    for other,channel_name,stream_url in match:
        if '{PQ},' in stream_url:
            ...
            if 'type' in other:
                ...
                if mode_type == 'yt-dl':
                    stream_url = stream_url +"&mode=1118000"
...
def getChannelItems(name,url,fanart):
        ...
        for channel in channel_list('subchannel'):
            ...
            try:
				if Adult_Pass == Adult_Default:
					addDir(name.encode('utf-8', 'ignore'),url.encode('utf-8'),1103000,thumbnail,fanArt,desc,genre,credits,date)
				else:
					if 'xxx' in name.lower():
					    pass
					elif 'adult' in name.lower():
            ...
            for a in all_items:
                ...
                if len(vurl) > 5 and TestName.find(Searchkey) > 0:
                    ...
                    if vurl.find('sublink') > 0:
                        addDir(name,vurl,1130000,thumbnail,fanart,'','','','')
                    else: 
                    ...
                    for i in url:
                    	if addon.getSetting('ask_playlist_items') == 'true':
	                        if regexs:
	                            ...
	                            playlist.append(i+'&mode=1119000')                            
                        else:
                            playlist.append(i)
                    ...
                    if isXMLSource:
                        if Adult_Pass == Adult_Default:
                            ...
                            if 'xxx' in name.lower():
                                ...
                                addDir(name.encode('utf-8'),ext_url[0].encode('utf-8'),1101000,thumbnail,FANART,desc,genre,date,None,'source')
                    elif isJsonrpc:
                        if Adult_Pass == Adult_Default:
                            addDir(name.encode('utf-8'),ext_url[0],1153000,thumbnail,fanart,desc,genre,date,None,'source')
                        else:
                            if 'xxx' in name.lower():
                                pass
                            elif 'adult' in name.lower():
                        ...
                        addDir(name.encode('utf-8'),url[0],1130000,thumbnail,fanart,'','','','')
...
def play_playlist(name, mu_playlist):
        ...
        if addon.getSetting('ask_playlist_items') == 'true':
            ...
            if index >= 0:
                if "&mode=1119000" in mu_playlist[index]:
                    xbmc.Player().play(urlsolver(mu_playlist[index].replace('&mode=1119000','')))
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                if name in str(SOURCES):
                    contextMenu.append(('Remove from Sources','XBMC.RunPlugin(%s?mode=1108000&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
            elif showcontext == 'download':
                contextMenu.append(('Download','XBMC.RunPlugin(%s?url=%s&mode=1109000&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
            elif showcontext == 'fav':
                contextMenu.append(('Remove from GenieTv Favourites','XBMC.RunPlugin(%s?mode=1106000&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to GenieTv Favourites','XBMC.RunPlugin(%s?mode=1105000&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
    ...
    if site_name == 'history' :
        ...
        for name,search_term in match:
            if 'plugin://' in search_term:
                ...
                addDir(name+':'+search_term,name,1126000,icon,FANART,'','','','')
    if not search_term:    
        ...
        if i['filetype'] == 'file':
            ...
            if title:
                ...
                if name.lower().replace(' ','') in title.lower().replace(' ',''):
                    addDir('Search '+final_name,url,1153000,thumbnail,fanart,description,'',date,'')
            else:
                addDir(name,url,1153000,thumbnail,fanart,description,'',date,'')
            #xbmc.executebuiltin("Container.SetViewMode(500)")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if regexs: 
            ...
            contextMenu.append(('[COLOR white]!!Download Currently Playing!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1121000&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))           
            ...
            contextMenu.append(('[COLOR white]!!Download Currently Playing!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=21&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))           
        elif url.endswith('&mode=1118000'):
            url=url.replace('&mode=1118000','')
            mode = '1118000' 
          
            contextMenu.append(('[COLOR white]!!Download!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1123000&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name)))) 
            if addon.getSetting('dlaudioonly') == 'true':
                contextMenu.append(('!!Download [COLOR seablue]Audio!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=1124000&name=%s)'
                                        %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))                                     
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                u += "mode=1113000&name=%s&playlist=%s" %(urllib.quote_plus(name), urllib.quote_plus(str(playlist).replace(',','||')))
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from GenieTv Favourites','XBMC.RunPlugin(%s?mode=1106000&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
            elif not name in FAV:
                fav_params = (
                    '%s?mode=1105000&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=0'
                ...
                contextMenu.append(('Add to GenieTv Favourites','XBMC.RunPlugin(%s)' %fav_params))
            contextMenu.append(('Queue Item', 'RunPlugin(%s?mode=1)' % sys.argv[0]))
            liz.addContextMenuItems(contextMenu)
        ...
        if not playlist is None:
            if addon.getSetting('add_playlist') == "false":
                ...
                contextMenu_ = [
                    ('Play '+playlist_name+' PlayList','XBMC.RunPlugin(%s?mode=1113000&name=%s&playlist=%s)'
