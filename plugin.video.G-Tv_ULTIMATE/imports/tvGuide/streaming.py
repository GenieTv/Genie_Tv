...
class StreamsService(object):
    def __init__(self, addon):
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.G-Tv_ULTIMATE', 'addons.ini'))
            xbmc.log('[plugin.video.G-Tv_ULTIMATE] Xhoans addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.G-Tv_ULTIMATE', customFile.split('/')[-1]))
            xbmc.log('[plugin.video.G-Tv_ULTIMATE] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
